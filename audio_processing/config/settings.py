"""
Обновленный файл настроек приложения с поддержкой MinIO.
...
class Settings(BaseSettings):
    ...
    MINIO_ENDPOINT: str = Field(
        default="10.200.115.155:9000",
        description="Адрес MinIO сервера (хост:порт)"
    )
    ...
    MINIO_SECURE: bool = Field(
        default=False,
        description="Использовать HTTPS при подключении к MinIO"
    )

    MINIO_REGION: str = Field(
        default="us-east-1",
        description="Регион MinIO"
    )

    MINIO_TIMEOUT: int = Field(
        default=60,
        ge=10,
        le=300,
        description="Таймаут для операций MinIO (секунды)"
    )

    # MinIO Buckets
    MINIO_UPLOADS_BUCKET: str = Field(
        default="audio-uploads",
        description="Bucket для входящих аудиофайлов"
    )

    MINIO_PROCESSING_BUCKET: str = Field(
        default="audio-processing",
        description="Bucket для файлов в процессе обработки"
    )

    MINIO_PROCESSED_BUCKET: str = Field(
        default="audio-processed",
        description="Bucket для обработанных файлов"
    )

    MINIO_JSON_OUTPUT_BUCKET: str = Field(
        default="json-results",
        description="Bucket для JSON результатов"
    )

    # ========== НАСТРОЙКИ API ==========

    BACKEND_API_BASE: str = Field(
        default="http://10.200.115.155/api",
        description="Базовый URL для отправки результатов"
    )

    API_ENDPOINT: str = Field(
        default="http://10.200.115.155/api/conversations",
        description="Эндпоинт для отправки результатов транскрипции"
    )

    # ========== НАСТРОЙКИ ТРАНСКРИПЦИИ ==========

    USE_AUTHORIZATION: bool = Field(
        default=False,
        description="Использование авторизации"
    )

    TRANSCRIPTION_SERVICE_URL: str = Field(
        default="http://10.200.115.157:6183/spr/stt/big",
        description="URL сервиса транскрипции"
    )

    TRANSCRIPTION_SERVICE_BY_JOB_URL: str = Field(
        default="http://10.200.115.157:6183/spr/result",
        description="URL сервиса результата транскрипции"
    )

    AUTHORIZATION_SERVICE_URL: str = Field(
        default="http://10.200.115.157:6183/auth/access",
    ...
    def ensure_directories(self) -> None:
        """Создать все необходимые директории (только для локального хранилища)"""
...
# Создаем директории при импорте (если используется локальное хранилище)
