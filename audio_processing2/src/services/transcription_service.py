"""
Исправленный TranscriptionService с правильным endpoint для получения результатов.
API endpoint: GET /spr/result/{taskID}
Статусы: ready, waiting, not found, failed

Замените полностью src/services/transcription_service.py на этот файл.
"""
...
class TranscriptionService:
    ...
    def __init__(self):
        ...
        self.polling_interval = 30.0
        self.max_polling_attempts = 30
    ...
    async def _authenticate(self, retry: bool = True) -> str:
        ...
        С повторами и растущей задержкой.
        Каждая попытка: до 60 сек на авторизацию.
        """
    ...
    async def submit_transcription_job(self, file_path: str) -> Optional[str]:
        """
        Отправить файл на асинхронную обработку и получить Job ID (taskID).
        
        Возвращает taskID для проверки статуса позже, или None если ошибка.
        """
        try:
            file_size_mb = os.path.getsize(file_path) / (1024 * 1024)
            
            logger.info("transcription.async_job.submitting",
                       file_path=file_path,
            ...
            params = {
                'speakers': 1, 
                'speaker_counter': 0, 
                'async': 1,  # ← КЛЮЧЕВОЙ: 1 = асинхронная обработка
                'punctuation': 0,
                'normalization': 0, 
                'toxicity': 1, 
                'emotion': 1, 
            ...
            data.add_field('wav', file_data, 
                          filename=filename, 
                          content_type=f'audio/{self._get_file_extension(file_path)}')
            ...
            async with self.session.post(
                ...
                if response.status == 200:
                    ...
                    logger.debug("transcription.submit.raw_response",
                                job_response=result)
                    
                    # ✅ Извлекаем taskID (это ID для получения результата)
                    task_id = result.get('taskID')
                    
                    if not task_id:
                        logger.error("transcription.async_job.no_id_in_response",
                                   response=result)
                        return None
                    
                    logger.info("transcription.async_job.submitted",
                               file=filename,
                               task_id=task_id,
                               file_size_mb=round(file_size_mb, 2))
                    ...
                    logger.warning("transcription.token.expired", file_path=file_path)
                    ...
                    text = await response.text()
                    logger.error("transcription.async_job.api_error", 
                               file_path=file_path, 
                               status_code=response.status,
            ...
            logger.error("transcription.async_job.submission.error",
                        error=str(e),
                        file_path=file_path)
            return None
    ...
    async def poll_transcription_result(self, task_id: str, 
        ...
        Параметры:
        - task_id: ID задачи
        - max_attempts: максимум попыток (по умолчанию 30 = 1 минута)
